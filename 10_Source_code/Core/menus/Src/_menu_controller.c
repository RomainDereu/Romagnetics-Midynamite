...
// ---------------------
// Persisted select per menu page (index into rows)
// ---------------------
static uint8_t s_menu_selects[UI_SELECT_COUNT] = {0};
static uint8_t s_prev_selects[UI_SELECT_COUNT] = {0};

// ---------------------
// Per-frame active fields list
// ---------------------
...
const menu_controls_t menu_controls[SAVE_FIELD_COUNT] = {
    ...
    [MIDI_MODIFY_CHANGE_OR_SPLIT]        = {   WRAP,  no_update,             0, 0 },
    [MIDI_MODIFY_VELOCITY_TYPE]          = {   WRAP,  no_update,             0, 0 },
    ...
    [MIDI_TRANSPOSE_TRANSPOSE_TYPE]      = {   WRAP,  no_update,             0, 0 },
    ...
    [SETTINGS_FILTERED_CHANNELS]         = {   WRAP,  update_channel_filter, 1, CTRL_G_SETTINGS_FILTER },
};

// ---------------------
// Local helpers (order matters to avoid implicit decls)
// ---------------------
...
static ui_group_t root_group(ui_group_t g) {
    switch (g) {
        ...
        case UI_GROUP_MODIFY_VEL_FIXED:  return UI_GROUP_MODIFY_BOTH;
        case UI_GROUP_TRANSPOSE_SHIFT:
        case UI_GROUP_TRANSPOSE_SCALED:
        case UI_GROUP_TRANSPOSE_BOTH:    return UI_GROUP_TRANSPOSE_BOTH;
...
static inline ui_group_t select_group_for_field_id(uint32_t id) {
    switch (id) {
        ...
        case CTRL_G_MODIFY_VEL_FIXED:    return UI_GROUP_MODIFY_BOTH;

        case CTRL_G_TRANSPOSE_SHIFT:
        case CTRL_G_TRANSPOSE_SCALED:
        case CTRL_G_TRANSPOSE_BOTH:      return UI_GROUP_TRANSPOSE_BOTH;

        case CTRL_G_SETTINGS_GLOBAL1:
        case CTRL_G_SETTINGS_GLOBAL2:
        case CTRL_G_SETTINGS_FILTER:
        case CTRL_G_SETTINGS_ALL:
        case CTRL_G_SETTINGS_ABOUT:      return UI_GROUP_SETTINGS;
...
static uint32_t ctrl_active_groups_from_ui_group(ui_group_t requested) {
    ...
    switch (requested) {
        ...
        case UI_GROUP_MODIFY_BOTH: {
            ...
            if (page == MIDI_MODIFY_SPLIT) mask |= flag_from_id(CTRL_G_MODIFY_SPLIT);
            else                           mask |= flag_from_id(CTRL_G_MODIFY_CHANGE);

            int vel  = (int)save_get(MIDI_MODIFY_VELOCITY_TYPE);
            if (vel == MIDI_MODIFY_FIXED_VEL) mask |= flag_from_id(CTRL_G_MODIFY_VEL_FIXED);
            else                               mask |= flag_from_id(CTRL_G_MODIFY_VEL_CHANGED);
        } break;

        case UI_GROUP_TRANSPOSE_BOTH: {
            mask |= flag_from_id(CTRL_G_TRANSPOSE_BOTH);
            int t = (int)save_get(MIDI_TRANSPOSE_TRANSPOSE_TYPE);
            if (t == MIDI_TRANSPOSE_SHIFT) mask |= flag_from_id(CTRL_G_TRANSPOSE_SHIFT);
            else                            mask |= flag_from_id(CTRL_G_TRANSPOSE_SCALED);
        } break;
        ...
        case UI_GROUP_SETTINGS: {
            ...
            if      (sel < rows_g1)                 mask |= flag_from_id(CTRL_G_SETTINGS_GLOBAL1);
            else if (sel < (uint8_t)(rows_g1+rows_g2)) mask |= flag_from_id(CTRL_G_SETTINGS_GLOBAL2);
            else if (sel < rows_total)              mask |= flag_from_id(CTRL_G_SETTINGS_FILTER);
            else                                     mask |= flag_from_id(CTRL_G_SETTINGS_ABOUT);
        } break;
...
static uint8_t ctrl_row_count(const CtrlActiveList *list) {
    ...
    for (uint8_t i = 0; i < list->count; ++i) {
        ...
        rows = (uint8_t)(rows + (is_bits_item(f) ? 16u : 1u));
    }
    return rows;
}
...
uint8_t menu_nav_get_select(menu_list_t field) {
    return (field < UI_SELECT_COUNT) ? s_menu_selects[field] : 0;
}
...
static void menu_nav_begin_and_update(menu_list_t field) {
...
static inline save_field_t selector_for_press(const NavSel *s) {
    ...
    if (s->root == UI_GROUP_TRANSPOSE_BOTH) return MIDI_TRANSPOSE_TRANSPOSE_TYPE;
...
static void select_press_menu_change(menu_list_t sel_field) {
    ...
    s_menu_selects[sel_field] = 0; // reset cursor to first
...
static inline void clear_all_field_changed(void) {
    for (int i = 0; i < CHANGE_BITS_WORDS; ++i) s_field_change_bits[i] = 0;
}

static uint8_t has_menu_changed(menu_list_t field, uint8_t current_select) {
    const uint8_t old_select  = (field < UI_SELECT_COUNT) ? s_prev_selects[field] : 0;
    const uint8_t sel_changed = (field < UI_SELECT_COUNT) && (old_select != current_select);

    const uint8_t data_changed = any_field_changed();

    if (field < UI_SELECT_COUNT) s_menu_selects[field] = current_select;
...
// ---------------------
// Public: which groups are active this frame
// ---------------------
...
// ---------------------
// Main per-page update
// ---------------------
void update_menu(menu_list_t menu) {
    menu_list_t field = menu; // page enum doubles as field index

    menu_nav_begin_and_update(field);

    switch (menu) {
        case MIDI_TEMPO: {
            const uint32_t bpm = (uint32_t)save_get(MIDI_TEMPO_CURRENT_TEMPO);
            // 6,000,000 / (bpm * 24)
        ...
        case MIDI_MODIFY: {
            if (handle_menu_toggle(GPIOB, Btn1_Pin, Btn2_Pin)) {
                select_press_menu_change(field); // resets cursor to 0
            }
        } break;

        case SETTINGS: {
            saving_settings_ui();
            filter_controller(); // draw 16-channel strip when FILTER section is active
        } break;
...
static uint8_t ui_state_set(ui_state_field_t field, uint8_t v) {
    if (!ui_state_try_lock()) return 0;
    switch (field) {
        case UI_CURRENT_MENU: ui_state.current_menu = v; break;
        case UI_OLD_MENU:     ui_state.old_menu     = v; break;
        default: break;
    }
    ui_state_unlock();
    return 1;
