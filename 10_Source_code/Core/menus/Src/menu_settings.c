...
{
    // Ensure active list/selection is aligned for this frame
    menu_nav_begin_and_update(UI_SETTINGS_SELECT);

    const uint8_t rows = build_select_states(UI_GROUP_SETTINGS, 0, NULL, 0);
    ...
    if (current_select >= (SETTINGS_FIRST_GLOBAL1 - SETTINGS_START_MENU) &&
        ...
        const ui_element elems[] = {
            { UI_ELEM_TEXT, 0, message->start_menu,          UI_FONT_6x8, TEXT_LEFT_START, LINE_1_VERT },
            { UI_ELEM_ITEM, SETTINGS_START_MENU, (const char*)message->choices.menu_list,      UI_FONT_6x8, 70, LINE_1_VERT },

            { UI_ELEM_TEXT, 0, message->usb_midi,            UI_FONT_6x8, TEXT_LEFT_START, LINE_2_VERT },
            { UI_ELEM_ITEM, SETTINGS_SEND_USB,  (const char*)message->choices.usb_receive_send, UI_FONT_6x8, 70, LINE_2_VERT },

            { UI_ELEM_TEXT, 0, message->contrast,            UI_FONT_6x8, TEXT_LEFT_START, LINE_3_VERT },
            { UI_ELEM_ITEM, SETTINGS_BRIGHTNESS,(const char*)message->contrast_levels,      UI_FONT_6x8, 70, LINE_3_VERT },
        };
        ...
        const ui_element elems[] = {
            { UI_ELEM_TEXT, 0, message->MIDI_Thru,           UI_FONT_6x8, TEXT_LEFT_START, LINE_1_VERT },
            { UI_ELEM_ITEM, SETTINGS_MIDI_THRU,  (const char*)message->choices.off_on, UI_FONT_6x8, 80, LINE_1_VERT },

            { UI_ELEM_TEXT, 0, message->USB_Thru,            UI_FONT_6x8, TEXT_LEFT_START, LINE_2_VERT },
            { UI_ELEM_ITEM, SETTINGS_USB_THRU,   (const char*)message->choices.off_on, UI_FONT_6x8, 80, LINE_2_VERT },

            { UI_ELEM_TEXT, 0, message->MIDI_Filter,         UI_FONT_6x8, TEXT_LEFT_START, LINE_3_VERT },
            { UI_ELEM_ITEM, SETTINGS_CHANNEL_FILTER, (const char*)message->choices.off_on, UI_FONT_6x8, 80, LINE_3_VERT },
        };
        ...
        for (uint8_t i = 0; i < 16; i++) {
            const char *label = (mask & (1u << i)) ? "X" : message->one_to_sixteen[i];
        ...
        const ui_element elems[] = {
            { UI_ELEM_TEXT, 0, message->about_brand,   UI_FONT_6x8, TEXT_LEFT_START, LINE_1_VERT },
            { UI_ELEM_TEXT, 0, message->about_product, UI_FONT_6x8, TEXT_LEFT_START, LINE_2_VERT },
            { UI_ELEM_TEXT, 0, message->about_version, UI_FONT_6x8, TEXT_LEFT_START, LINE_3_VERT },
        };
        menu_ui_render(elems, sizeof(elems)/sizeof(elems[0]));
    }
